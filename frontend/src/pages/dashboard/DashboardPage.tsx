...
import {
  Layers, Building, Home, FileText, TrendingUp, AlertCircle, CheckCircle2, Clock, Lock
} from 'lucide-react'
...
export default function DashboardPage() {
  ...
  // Backed by the 'financial_reports' module (core/permissions.py
  // FinancialReportPermission), which is now per-user override-able — so
  // this reflects overrides too, not just the role default.
  ...
  // Only fired for roles the backend will actually let through — avoids the
  // 403s Admin/Billing Officer/Viewer were hitting on every dashboard load.
  ...
  return (
    <div>
      ...
      {/* Stats row — non-financial, visible to every role */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
        {hasFinancialAccess ? (
          ...
          <div className="card lg:col-span-2 flex items-center gap-3 text-surface-400">
            <Lock className="w-4 h-4 shrink-0" />
            <span className="text-sm">Revenue and outstanding-due totals require Accountant or Super Admin access.</span>
          </div>
        )}
        <div className="card">
      ...
      {hasFinancialAccess ? (
        ...
        <div className="card flex flex-col items-center justify-center gap-2 py-12 text-surface-400">
          <Lock className="w-6 h-6" />
          <p className="text-sm">Revenue trend and payment-method charts require Accountant or Super Admin access.</p>
        </div>
      )}
    </div>
  )
